...
function renderProjectPicker(records){
  ...
  select.innerHTML='<option value="">Meine Projekte</option>'+records.map(row=>`<option value="${escapeHtml(row.record_key)}" ${row.record_key===currentRecordKey?'selected':''}>${escapeHtml(row.title||'Brandschutzordnung')}</option>`).join('');

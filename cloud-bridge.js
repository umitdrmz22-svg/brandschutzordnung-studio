...
const titleOf=payload=>{
  ...
  return [meta.documentNo,meta.company,meta.site].filter(Boolean).join(' · ')||'Brandschutzordnung';
};
...
function renderProjectPicker(records){
  ...
  select.title='Gespeichertes Projekt öffnen';
  select.innerHTML='<option value="">Werk-Projekte</option>'+records.map(row=>`<option value="${escapeHtml(row.record_key)}" ${row.record_key===currentRecordKey?'selected':''}>${escapeHtml(row.title||'Brandschutzordnung')}</option>`).join('');
...
async function saveCloud(force=false){
  ...
  try{
    ...
    if(!updated){
      const row={organization_id:membership.organization_id,app_key:cfg.appKey,record_key:currentRecordKey,...common,owner_user_id:session.user.id};
...
function hookStorage(){
  ...
  if(original.__bsoCloudWrapped)return;
  ...
  wrapped.__bsoCloudWrapped=true;
...
export async function prepare(){
  ...
  if(actions){
    ...
    logout.className='btn ghost';logout.textContent='Abmelden';
